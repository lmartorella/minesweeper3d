...
void buildArrayAct (int numVertex, int numFaces, int numSides) {
	...
	vdataAct = malloc(sizeof(float) * 3 * propAct[0]);
	tindicesAct = malloc(sizeof(int) * 3 * propAct[1]);
	lindexAct = malloc (sizeof(int) * 3 * propAct[2]);
}
...
void buildArrayNew () {
	...
	vdataNew = malloc(sizeof(float) * 3 * propNew[0]);
	tindicesNew = malloc(sizeof(int) * 3 * propNew[1]);
	lindexNew = malloc (sizeof(int) * 3 * propNew[2]);
}
